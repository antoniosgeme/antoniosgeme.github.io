...
// The WebGL context is created without preserveDrawingBuffer, so the render
// cell's only draw is discarded before the canvas is first composited and the
// visualization stays blank until an input changes. Re-emitting an input once
// the controls exist forces the redraw, without altering any value.
let redrawTries = 0;
const forceInitialRedraw = () => {
  const input = document.querySelector("#observablehq-viewof-alpha_deg-a125070b input");
  if (input) return input.dispatchEvent(new Event("input", {bubbles: true}));
  if (++redrawTries < 120) requestAnimationFrame(forceInitialRedraw);
};
